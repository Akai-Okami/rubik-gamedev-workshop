...
func _input(event: InputEvent) -> void:
	...
	match (event as InputEventKey).keycode:
		...
		KEY_1: rotate_layer("X", 0, not shift)   # correcto: gira capa X=0
		KEY_2: rotate_layer("X", 1, not shift)   # correcto: gira capa X=1
		KEY_Q: rotate_layer("X", 0, not shift)   # debería ser ("Y", 0): Q ahora gira en X en lugar de Y
		KEY_A: rotate_layer("X", 0, not shift)   # debería ser ("Z", 0): A ahora gira en X en lugar de Z

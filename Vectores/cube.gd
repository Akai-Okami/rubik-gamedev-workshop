...
func _animate(affected: Array[Cubie], axis: String, clockwise: bool) -> void:
	...
	var rot_axis: Vector3    # vector que define hacia dónde gira la capa
	match axis:
		"X":
			pivot.position.x = _grid_pos(affected[0].x, 0, 0).x        # centra el pivote en el eje X
			rot_axis = Vector3.RIGHT                                     # (1,0,0): correcto para el eje X
		"Y":
			pivot.position.y = _grid_pos(0, affected[0].y, 0).y        # centra el pivote en el eje Y
			rot_axis = Vector3.RIGHT                                     # UP (0,1,0): las capas Y giran en dirección incorrecta
		"Z":
			pivot.position.z = _grid_pos(0, 0, affected[0].z).z        # centra el pivote en el eje Z
			rot_axis = Vector3.RIGHT                                     #  BACK (0,0,1): las capas Z también giran mal

...
func _build_cube() -> void:
	for x in CUBE_SIZE:
		for y in CUBE_SIZE:
			for z in CUBE_SIZE:
				var node := _make_cubie(x, y, z)
				add_child(node)
				cubies.append(Cubie.new(node, x, y, z))
...
func _is_face_visible(x: int, y: int, z: int, face: String) -> bool:
	match face:
		"X+": return x == CUBE_SIZE      # debería ser CUBE_SIZE - 1: la cara derecha ahora está fuera de rango, desaparece
		"X-": return x == CUBE_SIZE      # debería ser 0: la cara izquierda también fuera de rango, desaparece
		"Y+": return y == CUBE_SIZE - 1  # correcto: cara superior en su límite real
		"Y-": return y == 3              # hardcodeado: funciona en 4×4 pero se rompe si cambias CUBE_SIZE
		"Z+": return z == CUBE_SIZE      # debería ser CUBE_SIZE - 1: cara frontal fuera de rango, desaparece
		"Z-": return z == CUBE_SIZE      # debería ser 0: cara trasera fuera de rango, desaparece
	return true                          # debería ser false: ahora TODAS las caras interiores son visibles
